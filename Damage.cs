...
public class Damage : MonoBehaviour {
	...
	{	
		...
		if (damageOnCollision && collision.collider.tag != "Unvurnable") {
			if (this.tag == "PlayerBullet" && collision.gameObject.tag == "Player")	// if the player got hit with it's own bullets, ignore it
				return;
	...
	{
		if (continuousDamage) {
			if (collision.gameObject.tag == "Player" && collision.gameObject.GetComponentInChildren<Health> () != null) {	// is only triggered if whatever it hits is the player
